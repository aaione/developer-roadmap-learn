...
- Markdown 本地链接和图片路径检查：通过。检查范围包含 15 个 Markdown 文件；新增三篇都包含学习目标、概念解释、实践建议、常见误区和延伸阅读；每篇参考链接不少于 5 个；公开内容未出现本机绝对路径。
...
- 已检查页面：
  - `http://localhost:5173/ai-engineer/02-model-basics/inference`：侧边栏 `Model Basics` 顺序正确，文章标题、学习目标、概念解释、实践建议、常见误区、延伸阅读和上一页/下一页导航呈现正常。
