...
- 为什么这两个岗位在不同公司里会有重叠。
- 初学者该怎么根据自己的兴趣选择学习路线。
- 面试或项目分工时，如何用问题类型判断自己正在做哪类工作。

## 概念解释

developer-roadmap 原文把差异讲得很清楚：AI Engineer 更偏向使用预训练模型和已有 AI 工具来改善用户体验，重点是把 AI 以实用方式应用到产品里，而不是从零构建模型；AI Researcher 和 ML Engineer 更偏向创建新模型或发展 AI 理论。这个说法适合做第一层判断，但现实岗位会更混杂。
...
<div class="flow-branch" aria-label="AI Engineer 和 ML Engineer 分工判断">
  <span class="flow-question">需要从业务数据训练或微调模型吗？</span>
  <span class="flow-path"><span class="flow-step">是：更偏 ML Engineer / MLOps</span><span class="flow-step">否：继续判断</span></span>
  <span class="flow-question">主要是在组装模型、工具、上下文和产品流程吗？</span>
  <span class="flow-path"><span class="flow-step">是：更偏 AI Engineer</span><span class="flow-step">否：继续判断</span></span>
  <span class="flow-question">主要是研究新算法或新模型能力吗？</span>
  <span class="flow-path"><span class="flow-step">是：更偏 AI Researcher</span><span class="flow-step">否：可能是后端、数据工程或产品工程问题</span></span>
</div>

在小团队里，一个人可能同时做两边。比如你既要接 OpenAI 或 Claude API 做问答，也要训练一个内部分类模型。这时岗位标题不重要，重要的是你知道自己正在解决哪类问题。Codecademy、Coursera、TechTarget、IBM 和 CSP Global 的文章都反复提到同一点：AI Engineer 和 ML Engineer 都要懂编程和数据，但前者更靠近产品集成，后者更靠近模型生命周期。
